...
def _search_single_restaurant(loc: str, api_key: str) -> dict:
    ...
    headers = {
        "Content-Type": "application/json",
        "X-Goog-Api-Key": api_key,
        "X-Goog-FieldMask": "places.displayName,places.formattedAddress,places.rating,places.primaryType,places.priceLevel,places.userRatingCount"
    }
    body = {
        "textQuery": f"highly-rated local favorite restaurants, eateries, and local dining in {loc}"
    }
...
def search_restaurants(location: str) -> dict:
    """Searches for highly-rated local favorite restaurants and eateries at a destination using the Google Places API (New).
    ...
    Returns:
        A dictionary listing restaurants with names, ratings, addresses, price levels, and styles.
    """
    args_dict = {"location": location}
    api_key = get_google_maps_api_key()
    if not api_key or api_key == "YOUR_MAPS_API_KEY":
