...
def _search_single_restaurant(loc: str, api_key: str) -> dict:
    ...
    body = {
        "textQuery": f"best restaurants, cafes, fine dining, and local food spots in {loc}"
    }
...
def search_restaurants(location: str) -> dict:
    """Searches for top restaurants, cafes, and eateries at a location using the Google Places API (New).
    ...
    Returns:
        A dictionary listing restaurants with names, ratings, addresses, and price levels.
    """
    args_dict = {"location": location}
    api_key = get_google_maps_api_key()
    if not api_key or api_key == "YOUR_MAPS_API_KEY":
